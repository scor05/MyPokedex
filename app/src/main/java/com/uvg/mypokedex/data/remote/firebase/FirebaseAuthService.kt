...
class FirebaseAuthService {
    ...
    // Timeout de 30 segundos para operaciones de Firebase
    private val OPERATION_TIMEOUT = 30000L
    ...
    suspend fun signInAnonymously(): Result<FirebaseUser> {
        return try {
            val result = withTimeout(OPERATION_TIMEOUT) {
            ...
            Result.failure(Exception("Timeout: La conexión está tardando demasiado. Verifica tu internet."))
        } catch (e: Exception) {
    ...
    // Login Anónimo con Alias Personalizado
    ...
    private suspend fun createUserDocument(uid: String, displayName: String, email: String? = null) {
        try {
            ...
            println("⚠️ Error creando documento de usuario: ${e.message}")
        }
    }

    // Obtener datos del usuario desde Firestor
    suspend fun getUserData(uid: String): Result<UserDto> {
        return try {
            val document = withTimeout(OPERATION_TIMEOUT) {
                firestore.collection("users")
                    .document(uid)
                    .get()
                    .await()
            ...
            Result.failure(Exception("Timeout al obtener datos del usuario"))
        } catch (e: Exception) {
            Result.failure(e)
        }
    }
    ...
    suspend fun updateDisplayName(uid: String, newName: String): Result<Unit> {
        return try {
            ...
            Result.failure(Exception("Timeout al actualizar nombre"))
        } catch (e: Exception) {
            Result.failure(e)
        }
    }
